...
export const Navbar: React.FC<NavbarProps> = ({ 
  ...
  return (
    <>
    {/* Z-Index 50 to stay above sticky headers. Responsive height: h-16 (mobile), h-20 (desktop) */}
    <nav className="fixed top-0 left-0 right-0 z-[50] bg-white/80 dark:bg-black/60 backdrop-blur-xl border-b border-white/20 dark:border-white/10 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16 md:h-20 transition-all duration-300">
            ...
            {/* Branding & User Role */}
            <div className="flex items-center gap-3 md:gap-4 group cursor-default max-w-[70%]">
                ...
                <div className="flex flex-col min-w-0">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-0.5 sm:gap-1.5 mt-0.5 md:mt-1">
                        ...
                        {user.siteId && (
                             <span className="inline-flex px-1.5 py-0.5 rounded-md bg-slate-100 dark:bg-white/10 text-[9px] font-mono text-slate-600 dark:text-slate-300 truncate max-w-[120px]">
                                Site: {user.siteId.substring(0, 8)}...
                             </span>
                        )}
                    </div>
                </div>
            ...
            <div className="flex items-center gap-1 md:gap-3">
                ...
                <button 
                    onClick={onLogout}
                    className="p-2 md:px-4 md:py-2 flex items-center gap-2 text-slate-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition-all"
                    title="Logout"
                >
    ...
    {/* Responsive spacer to push content below fixed navbar */}
