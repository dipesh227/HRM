...
export const Card: React.FC<CardProps> = ({ children, className = '', title, subtitle, actions, noPadding = false }) => {
  return (
    <div className={`bg-white dark:bg-ios-dark-card rounded-2xl md:rounded-3xl shadow-ios dark:shadow-none border border-slate-100 dark:border-white/5 overflow-hidden transition-all duration-300 ${className}`}>
      {(title || actions) && (
        <div className="px-4 py-4 md:px-6 md:py-5 border-b border-slate-50 dark:border-white/5 flex flex-col sm:flex-row justify-between sm:items-center gap-3 bg-white/50 dark:bg-white/5 backdrop-blur-sm">
          <div>
            {title && <h3 className="font-bold text-lg text-slate-900 dark:text-white tracking-tight leading-snug">{title}</h3>}
