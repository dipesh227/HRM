...
import { Copy, Check, AlertTriangle, ExternalLink, RefreshCw, Database, Server, Code, ShieldCheck, Trash2 } from 'lucide-react';

export const DatabaseSetup: React.FC<{ onRetry: () => void, error: string, errorCode?: string }> = ({ onRetry, error, errorCode }) => {
  const [copied, setCopied] = useState(false);

  // SQL Schema Content - Standard v1.0 (Reverted)
  const schema = `-- KONARK HR SYSTEM - STANDARD SCHEMA (NO AES) v1.0
...
ON CONFLICT (title) DO NOTHING;`;
  ...
  const handleFactoryReset = () => {
    if (confirm("This will clear all local app data and reload. Are you sure?")) {
        localStorage.clear();
  ...
  const isSchemaError = errorCode === 'NO_SCHEMA' || errorCode === '42P01'; // 42P01 is table missing
  ...
  return (
    <div className="fixed inset-0 bg-slate-900 z-[100] flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] border border-slate-200 dark:border-slate-800">
        {/* Header */}
        <div className="bg-blue-50 dark:bg-blue-900/20 p-6 border-b border-blue-100 dark:border-blue-900/30 flex items-start gap-4">
            <div className="p-3 bg-blue-100 dark:bg-blue-900/40 rounded-full text-blue-600 dark:text-blue-400 shrink-0">
                <Database className="w-6 h-6" />
            </div>
            <div>
                <h2 className="text-xl font-bold text-slate-800 dark:text-white">Standard Database Schema (v1.0)</h2>
                <p className="text-slate-600 dark:text-slate-300 mt-1">
                    Running in Standard Mode. AES Encryption has been removed for compatibility.
                </p>
                <div className="mt-2 text-xs font-mono bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-500 p-2 rounded max-w-xl truncate">
                    Status Code: {errorCode || 'INITIAL'} | {error}
                </div>
            </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8 bg-white dark:bg-slate-900">
            ...
            <div className={`flex gap-4 transition-opacity ${isSchemaError ? 'opacity-50' : 'opacity-100'}`}>
                ...
                <div className="pb-8">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mb-2">Check environment variables.</p>
            ...
            <div className={`flex gap-4 transition-opacity ${!isSchemaError && !isAuthError ? 'opacity-100' : isAuthError ? 'opacity-50' : 'opacity-100'}`}>
                <div className="flex flex-col items-center gap-2">
                    <div className={`w-8 h-8 rounded-full bg-slate-800 dark:bg-slate-700 text-white flex items-center justify-center font-bold ${isSchemaError ? 'animate-pulse ring-4 ring-blue-100 dark:ring-blue-900/30' : ''}`}>2</div>
                </div>
                <div className="pb-8 w-full">
                    <h3 className="font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2">
                        <Code className="w-5 h-5 text-blue-500" />
                        Run Standard SQL Schema (v1.0)
                    </h3>
                    <p className="text-slate-500 dark:text-slate-400 mb-3">
                        Copy this SQL and run it in your <strong>Supabase Studio SQL Editor</strong>. This resets the DB to standard columns.
                    </p>
        ...
        {/* Footer */}
        <div className="bg-slate-50 dark:bg-slate-900 p-6 border-t border-slate-200 dark:border-slate-800 flex justify-between gap-3">
             <button onClick={handleFactoryReset} className="flex items-center gap-2 px-4 py-3 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg text-sm font-bold transition-colors">
